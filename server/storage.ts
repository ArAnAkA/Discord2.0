...
export interface IStorage {
  ...
  getServerMembers(serverId: number): Promise<(ServerMember & { user: User })[]>;
  ...
  // Messages
  getMessages(channelId: number): Promise<(Message & { sender: User })[]>;
  createMessage(message: InsertMessage & { userId: number }): Promise<Message & { sender: User }>;
}
...
export class DatabaseStorage implements IStorage {
  ...
  async getServerMembers(serverId: number): Promise<(ServerMember & { user: User })[]> {
  ...
  async getMessages(channelId: number): Promise<(Message & { sender: User })[]> {
    const rows = await db.select({
      message: messages,
      user: users
    })
    ...
    return rows.map(r => ({ ...r.message, sender: r.user }));
  }

  async createMessage(message: InsertMessage & { userId: number }): Promise<Message & { sender: User }> {
    const [newMessage] = await db.insert(messages).values(message).returning();
    const user = await this.getUser(message.userId);
    if (!user) throw new Error("User not found");
    return { ...newMessage, sender: user };
  }
}
